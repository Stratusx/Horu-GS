...
function midcast(spell,arg)
	...
	if windower.wc_match(spell.name,'Utsusemi*') then
		...
		if spell.name == 'Utsusemi: Ichi' and ShadowType == 'Ni' or ShadowType == 'San' then
...
-- Utsusemi Variable Sets
    ...
    elseif spell and spell.name == 'Utsusemi: Ichi' then
        ShadowType = 'Ichi'
	elseif spell and spell.name == 'Utsusemi: San' then
        ShadowType = 'San'
	end
end
	...
	if TH == 1 then
		...
		if buffactive["Feint"] then
			...
			elseif Mode == 3 then
				...
				windower.add_to_chat(121,'Feint - Hybrid Evasion Set')
			elseif Mode == 4 then
				equip(sets.idle.Evasion,{legs="Plunderer's Culottes"})
				windower.add_to_chat(121,'Feint - Full Evasion Set')
			end		
		else
			if Mode == 0 then
				equip(sets.TP)
